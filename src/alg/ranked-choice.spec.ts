...
const randInt = (min = 0, max: number) => min + Math.floor(Math.random() * (max - min + 1)); 

const randFloat = (min = 0, max: number) => min + Math.random() * (max - min); 

const choose = <T>(opts: T[]) => {
    console.assert(opts.length > 0, 'Cannot choose random element from empty list');
...
const shuffleWithFirst = (opts: TestOpt[], firstPlace: string) => {
    ...
    console.assert(!!first, `shuffleWithFirst: ${firstPlace} not found in provided options`);

    
    const notFirst = opts
        .filter(({ id }) => id !== firstPlace);
...
const options = (len = 10) => [...Array(len)]
    .map((_ , i) => ({ id: String.fromCharCode(i + 49)} ));
...
it.each(times(20))('chooses outright majority', () => {
    ...
    const rankings = [...Array(voters)]
        .map((_, i) => i <= majority ?
            shuffleWithFirst(opts, winner.id) :
            shuffleWithNotInFirst(opts, winner.id));
    console.log('test: first places', rankings.map(r => r[0]));
...
it.skip('finds winner after 1 runoff', () => {
    ...
    const majority = randInt(
        Math.ceil(opts.length / 1) + 1,
        opts.length
    );

    // Now split majority
    const portion = majority === opts.length ? randFloat(0.25, 0.75) : randFloat(0, 0.9);
    ...
    const firstRoundFirsts = times(rounds[0])
        .map(() => shuffleWithFirst(opts, winner.id));
